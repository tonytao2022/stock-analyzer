...
def main():
    ...
    for t in all_trades:
        t.setdefault('exit_reason', '未知')
        exit_groups[t['exit_reason']].append(t)
    
    print(f"\n{'='*90}")
    print(f"📊 按卖出原因分组统计")
    print(f"{'='*90}")
    reason_order = ['信号卖出', '强制平仓(60日)', '期末平仓']
    print(f"\n{'卖出原因':20s} {'交易数':>6s} {'胜数':>6s} {'败数':>6s} {'胜率':>7s} {'均收益':>9s} {'中位收益':>10s} {'平均持有':>7s}")
    print(f"{'─'*75}")
    for reason in reason_order:
        ...
        median = sorted_p[n//2]
        avg_days = sum(t['hold_days'] for t in trades)/n
        print(f"  {reason:20s} {n:>6d} {wins:>6d} {n-wins:>6d} {wins/n*100:>6.1f}% {avg:>+8.2f}% {median:>+9.2f}% {avg_days:>6.1f}")
    
    # 持有天数×卖点交叉分析
    print(f"\n{'─'*90}")
    print(f"📊 持有天数 × 卖点 交叉分析")
    print(f"{'─'*90}")
    day_ranges = [(1,3),(4,7),(8,10),(11,15),(16,20),(21,30),(31,45),(46,60)]
    print(f"\n{'持有天数':10s} {'信号卖出':>28s} {'强制平仓(60日)':>28s}")
    print(f"{'':10s} {'交易数':>6s}{'胜率':>8s}{'均收益':>10s} {'交易数':>8s}{'胜率':>8s}{'均收益':>10s}")
    print(f"{'─'*80}")
    for lo, hi in day_ranges:
        sig = [t for t in all_trades if t.get('exit_reason')=='信号卖出' and lo<=t['hold_days']<=hi]
        fq = [t for t in all_trades if t.get('exit_reason')=='强制平仓(60日)' and lo<=t['hold_days']<=hi]
        out = f"  {f'{lo}-{hi}日':10s}"
        for tr in [sig, fq]:
            if tr:
                n=len(tr); w=sum(1 for p in tr if p['profit_pct']>0); a=sum(t['profit_pct'] for t in tr)/n
                out += f" {n:>5d} {w/n*100:>6.1f}% {a:>+8.2f}%"
            else:
                out += f" {'':>6s} {'':>8s} {'':>10s}"
        print(out)
    
    print(f"\n{'='*90}")
    print(f"📊 信号跟踪回测报告 v1.0 — 真实交易模拟")
