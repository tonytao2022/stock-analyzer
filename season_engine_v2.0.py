...
判定维度：6维度 >15项指标
输出：spring/summer/autumn/winter/chaos + confidence(0-1) + rule_chain
...
class SeasonJudge:
    ...
    def judge_at(self, idx: int, market_breadth_mapped: Optional[Dict] = None) -> Tuple[str, float, Dict]:
        ...
        # 季节映射
        season = self._score_to_season(raw_score)
        ...
        details = {
            ...
            'rule_chain': self._build_rule_chain(dimensions, season, raw_score, confidence),
        }
    ...
    def _build_rule_chain(dimensions: List[DimensionResult], season: str,
                          raw_score: float, confidence: float) -> str:
        """构建可解释的规则链"""
